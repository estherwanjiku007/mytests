...
function calcNssfDed(){
    let pensionScheme;
    let maxPensionablePay;
    ...
    function calcFringeBenefit(){
        let month;
        let interest;
        if(month===January||February||March){
            console.log(interest<=0.09)
        }
        else if(month===April||May ||June){
            console.log(interest<=0.1)
        }
        else if(mmonth===July||August||September){
            console.log(interest<=0.11)
        }
        else {
            console.log(interest<=0.13)
        }
        function calcHousingFund(basicSalary){
            let housingLevy=basicSalary*0.03
           console.log("housingLevy")
        }
    }
    function calcNetSalary(basicSalary){
        let netSalary;
     let deductions=loans+interest+nhifDed+payee+maxContributions+(housingLevy/2);
